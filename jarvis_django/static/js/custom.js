// JavaScript Document
	...
	*/
	...
	$(document).ready(function(){
		...
		$('#LogoLink, #GoToMainContent, #GoToHome, #GoToFeatures,#GoToHow,#GoToSub,#purchase,#GoToGallery,#GoToNextFeature1, #GoToScreenshorts, #GoToNextFeature2, #GoToVideoReview, #GoToCallToAction1, #GoToCallToAction2, #GoToVideo, #GoToInsights, #GoToSubscribe' ).scrollTo({ speed: 1400 });
	...
	function parallaxInit() {
		...
		$('#testimonials-rotator').parallax("10%", 0.3);
	...
	$(document).ready(function(){
        ...
        $('#subscribe-submit').on("click", function () {
                var data = {
                    Body: "Marketly contact requested for user " + $('#email').val(),
                    Subject: "New Marketly Lead",
                    ToList: ["contact@cloudgust.com"]
                };
                $('#emailbody').hide(); $('#textbody').show();
                $.ajax({
                                url: 'https://www.bizkinetic.com/api/servermailer',
                                beforeSend: function(xhr) {xhr.setRequestHeader('Authorization', "Basic YnVpbGRlckBkZW1vLmNvbTpkZW1vMTIzNA==");  },
                                data: data,
                                dataType: 'json',
                                type: 'POST',
                                success: function() { return;},
                                error: function(err) { console.log(err); return; }
                            });
            });
